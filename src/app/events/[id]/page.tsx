    ...
    <div style={{ position:'fixed', inset:0, zIndex:100, display:'flex', alignItems:'center', justifyContent:'center' }}>
      ...
      <div style={{ position:'relative', width:'100%', maxWidth:400, margin:'0 16px', borderRadius:24, padding:28, background:C.card, border:`1px solid ${C.greenDim}`, boxShadow:'0 20px 60px rgba(16,185,129,0.15)' }}>
        <div style={{ textAlign:'center' }}>
          ...
          <h2 style={{ fontSize:20, fontWeight:800, color:C.text, fontFamily:'Sora,sans-serif', marginBottom:8 }}>Seat Confirmed! 🎉</h2>
          <p style={{ fontSize:14, color:C.textMuted, fontFamily:'Inter,sans-serif', lineHeight:1.6, marginBottom:20 }}>
          ...
          <div style={{ display:'flex', gap:10 }}>
            <Link href="/dashboard/tickets" style={{ flex:1, textDecoration:'none' }}>
              <button style={{ width:'100%', padding:'11px', borderRadius:12, border:`1px solid ${C.border}`, background:C.surface, color:C.textMuted, cursor:'pointer', fontFamily:'Inter,sans-serif', fontWeight:600, fontSize:14 }}>
                My Tickets
              </button>
            </Link>
            <button onClick={onClose} style={{ flex:1, padding:'11px', borderRadius:12, border:'none', background:C.green, color:'#fff', cursor:'pointer', fontFamily:'Inter,sans-serif', fontWeight:700, fontSize:14 }}>
              Done
            </button>
          </div>
        </div>
...
export default function EventDetailPage() {
  ...
  if (!event) return (
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:C.bg }}>
      <div style={{ textAlign:'center' }}>
        <p style={{ fontSize:18, color:C.textMuted, fontFamily:'Inter,sans-serif' }}>Event not found</p>
  ...
  return (
    <div style={{ background:C.bg, minHeight:'100vh' }}>
      ...
      <div style={{ padding:'16px 24px', borderBottom:`1px solid ${C.border}`, display:'flex', alignItems:'center', gap:12 }}>
        <button onClick={() => router.back()} style={{ display:'flex', alignItems:'center', gap:6, background:'none', border:'none', cursor:'pointer', color:C.textMuted, fontFamily:'Inter,sans-serif', fontSize:14 }}>
          <ChevronLeft style={{ width:16, height:16 }} /> Back
        </button>
        <span style={{ color:C.textDim }}>·</span>
        <span style={{ fontSize:14, color:C.textMuted, fontFamily:'Inter,sans-serif', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{event.title}</span>
      </div>
        ...
        <div className="event-grid" style={{ display:'grid', gridTemplateColumns:'1fr 300px', gap:24, alignItems:'start' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
            ...
            <div>
              <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:10 }}>
                ...
                {event.category && (
                  <span style={{ fontSize:12, padding:'4px 10px', borderRadius:20, background:C.blueDim, color:C.blueLight, fontFamily:'Inter,sans-serif' }}>{event.category}</span>
                )}
              </div>
              <h1 style={{ fontSize:'clamp(20px, 5vw, 28px)', fontWeight:800, color:C.text, fontFamily:'Sora,sans-serif', letterSpacing:'-0.02em', marginBottom:12 }}>{event.title}</h1>
              {event.description && (
                <p style={{ fontSize:15, color:C.textMuted, fontFamily:'Inter,sans-serif', lineHeight:1.75 }}>{event.description}</p>
              )}
            </div>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
              {[
                ...
                <div key={i} style={{ display:'flex', alignItems:'center', gap:10 }}>
                  ...
                  <span style={{ fontSize:14, color:C.textMuted, fontFamily:'Inter,sans-serif' }}>{item.label}</span>
                </div>
              ))}
            </div>
            ...
            <div style={{ padding:16, borderRadius:16, background:C.card, border:`1px solid ${C.border}` }}>
              <p style={{ fontSize:11, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.1em', color:C.textDim, fontFamily:'Inter,sans-serif', marginBottom:12 }}>Hosted by</p>
              <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                <div style={{ width:48, height:48, borderRadius:'50%', overflow:'hidden', background:avatarColor(host.id||'')+'22', display:'flex', alignItems:'center', justifyContent:'center', fontSize:18, fontWeight:700, color:avatarColor(host.id||''), fontFamily:'Sora,sans-serif', flexShrink:0 }}>
                  {host.photo_url ? <img src={host.photo_url} alt="" style={{ width:'100%', height:'100%', objectFit:'cover' }} /> : getName(host).slice(0,2).toUpperCase()}
                </div>
                <div>
                  <p style={{ fontSize:15, fontWeight:700, color:C.text, fontFamily:'Sora,sans-serif' }}>{getName(host)}</p>
                  <p style={{ fontSize:12, color:C.textMuted, fontFamily:'Inter,sans-serif' }}>Host · GritClub</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="event-ticket-panel" style={{ position:'sticky', top:24 }}>
            <div style={{ borderRadius:20, padding:20, background:C.card, border:`1px solid ${isLive?'rgba(239,68,68,0.3)':C.border}`, display:'flex', flexDirection:'column', gap:16 }}>
              ...
              {/* Price */}
              <div>
                <p style={{ fontSize:11, fontWeight:600, textTransform:'uppercase', letterSpacing:'0.1em', color:C.textDim, fontFamily:'Inter,sans-serif', marginBottom:6 }}>
                  {hasTicket ? 'Your Ticket' : 'Ticket Price'}
                </p>
                <p style={{ fontSize:32, fontWeight:800, color:isFree?C.green:C.gold, fontFamily:'Sora,sans-serif', letterSpacing:'-0.03em' }}>
                  {isFree ? 'Free' : fmt(event.price || 0)}
                </p>
                {!isFree && <p style={{ fontSize:12, color:C.textDim, fontFamily:'Inter,sans-serif', marginTop:2 }}>per ticket · secure payment</p>}
              </div>
              ...
              {event.capacity > 0 && (
                <div>
                  <div style={{ display:'flex', justifyContent:'space-between', marginBottom:6 }}>
                    ...
                    <span style={{ fontSize:12, color:C.textDim, fontFamily:'Inter,sans-serif' }}>{fillPct}% full</span>
                  </div>
              ...
              {hasTicket ? (
                <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:8, padding:'12px 16px', borderRadius:12, background:C.greenDim, border:`1px solid rgba(16,185,129,0.3)` }}>
                    ...
                    <span style={{ fontSize:14, fontWeight:700, color:C.green, fontFamily:'Inter,sans-serif' }}>Seat Confirmed ✓</span>
                  </div>
                  {isLive && (
                    <button onClick={joinLive}
                      style={{ width:'100%', padding:'14px', borderRadius:12, border:'none', cursor:'pointer', background:C.red, color:'#fff', fontFamily:'Inter,sans-serif', fontWeight:800, fontSize:16, display:'flex', alignItems:'center', justifyContent:'center', gap:8 }}>
                      <Radio style={{ width:18, height:18 }} /> Join Live Now →
                    </button>
                  )}
                </div>
              ) : soldOut ? (
                <button disabled style={{ width:'100%', padding:'14px', borderRadius:12, border:'none', background:C.border, color:C.textDim, fontFamily:'Inter,sans-serif', fontWeight:700, fontSize:16, cursor:'not-allowed' }}>
                  Sold Out
                </button>
              ) : isFree ? (
                <button onClick={claimFreeTicket} disabled={claiming}
                  style={{ width:'100%', padding:'14px', borderRadius:12, border:'none', cursor:claiming?'wait':'pointer', background:`linear-gradient(135deg, ${C.green}, #059669)`, color:'#fff', fontFamily:'Inter,sans-serif', fontWeight:800, fontSize:16, display:'flex', alignItems:'center', justifyContent:'center', gap:8, opacity:claiming?0.7:1, boxShadow:'0 4px 20px rgba(16,185,129,0.3)' }}>
              ...
              {!hasTicket && !user && (
                <p style={{ fontSize:12, color:C.textDim, fontFamily:'Inter,sans-serif', textAlign:'center' }}>
              ...
              <button onClick={() => navigator.clipboard?.writeText(window.location.href)}
                style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:6, padding:'10px', borderRadius:12, border:`1px solid ${C.border}`, background:'transparent', color:C.textMuted, cursor:'pointer', fontFamily:'Inter,sans-serif', fontSize:13 }}>
