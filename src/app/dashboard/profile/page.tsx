...
import { Save, Ticket, Users, Radio, Edit2, CheckCircle, LogOut } from 'lucide-react'
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setProfile(prof)
      setForm({
        profile_bio: prof?.profile_bio || '',
        photo_url: prof?.photo_url || u.user_metadata?.avatar_url || ''
      })

      const { count: ticketCount } = await supabase
        .from('tickets').select('*', { count: 'exact', head: true })
        .eq('user_id', u.id).eq('status', 'paid')

      const { count: eventCount } = await supabase
        .from('events').select('*', { count: 'exact', head: true })
        .eq('host_id', u.id)

      setStats({ tickets: ticketCount || 0, connections: 0, events: eventCount || 0 })
    }
    load()
  }, [])
  ...
  const handleSave = async () => {
    ...
    const { error } = await supabase.from('users')
      .update({ profile_bio: form.profile_bio, photo_url: form.photo_url })
      .eq('id', user.id)
    if (!error) {
      setProfile((p: any) => ({ ...p, profile_bio: form.profile_bio, photo_url: form.photo_url }))
      setSaved(true)
      setEditing(false)
    ...
    <DashboardLayout>
      <div className="p-4 md:p-6 max-w-2xl mx-auto">
        ...
        <div className="rounded-2xl overflow-hidden mb-4" style={{ background: '#1E293B' }}>
          <div className="h-24" style={{ background: 'linear-gradient(135deg, #0F172A 0%, #1E3A5F 50%, #0F172A 100%)' }} />
          <div className="px-5 pb-5">
            <div className="flex items-end justify-between -mt-8 mb-4">
              <div className="w-16 h-16 rounded-full overflow-hidden ring-4 ring-slate-900 flex items-center justify-center text-xl font-bold flex-shrink-0"
                style={{ background: 'linear-gradient(135deg, #38BDF8, #0EA5E9)', color: '#0F172A' }}>
                {(form.photo_url || profile?.photo_url)
                  ? <img src={form.photo_url || profile?.photo_url} alt="" className="w-full h-full object-cover" />
                  : initials}
              </div>
              <button onClick={() => setEditing(!editing)}
                className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium"
                style={{ background: editing ? 'rgba(56,189,248,0.15)' : '#334155', color: editing ? '#38BDF8' : '#E2E8F0' }}>
            ...
            <div className="mb-4">
              ...
              {profile?.profile_bio && !editing && (
                <p className="text-sm text-slate-300 mt-2 leading-relaxed">{profile.profile_bio}</p>
              )}
            </div>
            ...
            {/* Edit form */}
            {editing && (
              <div className="space-y-3 pt-4" style={{ borderTop: '1px solid #334155' }}>
                <div>
                  ...
                  <input
                    ...
                    placeholder="https://..."
                    className="w-full px-4 py-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-sky-500"
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full px-4 py-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:ring-sky-500 resize-none"
                    style={{ background: '#0F172A', border: '1px solid #334155', color: '#E2E8F0' }}
                  />
                </div>
        ...
        {profile?.role === 'audience' && (
          <div className="rounded-2xl p-5 mb-4"
            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <div className="font-semibold text-sm text-white">Become a Host</div>
                <div className="text-xs text-slate-400">Earn 80% on every ticket sold</div>
              </div>
            </div>
