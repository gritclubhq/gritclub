...
// ─── Whiteboard ─────────────────────────────────────────────────────────────
// NOTE: toolbar is rendered OUTSIDE the video overlay — it sits in the bottom bar
// canvasRef is passed from parent so parent can call captureStream() on it
function WhiteboardCanvas({ bg, canvasRef }: { bg: string; canvasRef: React.RefObject<HTMLCanvasElement> }) {
  const drawing = useRef(false)
  const origin  = useRef<{x:number,y:number}|null>(null)
  const snap    = useRef<ImageData|null>(null)
  // These are controlled by the toolbar in the parent via callbacks
  ...
  const down = (e: any) => {
    ...
    if (toolRef.current === 'pen' || toolRef.current === 'eraser') { ctx.beginPath(); ctx.moveTo(p.x, p.y) }
  }
  const move = (e: any) => {
    ...
    const c = canvasRef.current; if (!c) return
    const ctx = c.getContext('2d')!, p = xy(e, c)
    const tool = toolRef.current, color = colorRef.current, size = sizeRef.current
...
// ─── Whiteboard Toolbar (rendered in the bottom bar, not over video) ──────────
function WhiteboardToolbar({
  bg, canvasRef, onClear,
  tool, setTool, color, setColor, size, setSize
}: {
  bg: string; canvasRef: React.RefObject<HTMLCanvasElement>; onClear: () => void
      ...
      <div style={{display:'flex',gap:2,background:C.card,padding:3,borderRadius:8}}>
        {TOOLS.map(({id,I}:any) => (
          <button key={id} onClick={()=>setTool(id)} title={id}
            style={{width:28,height:28,borderRadius:6,border:'none',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',background:tool===id?C.blue:'transparent',color:tool===id?'#fff':C.textMuted}}>
...
export default function LiveRoomPage() {
  ...
  const [needsUnmute,  setNeedsUnmute] = useState(false) // viewer tap-to-unmute prompt
  ...
  // WebRTC — viewer has a single connection to host
  ...
  // Close whiteboard bg dropdown on outside click
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      if (ml) {
        const now = new Date()
        setMutedUsers(new Set(ml.filter(m => !m.muted_until || new Date(m.muted_until) > now).map(m => m.user_id)) as Set<string>)
      }
  ...
  // Replace video sender track on all viewer peer connections
  ...
  // ── HOST: make one RTCPeerConnection per viewer ─────────────────────
  const makeHostPeer = (viewerId: string, camStreamRef: MediaStream) => {
    ...
    // Add BOTH video and audio tracks from camera stream
    // Audio track stays permanently — never replaced — so viewers always hear mic
    camStreamRef.getTracks().forEach(t => pc.addTrack(t, camStreamRef))

    pc.onicecandidate = ({ candidate }) => {
      if (!candidate || !sigCh.current) return
      sigCh.current.send({ type:'broadcast', event:'ice', payload:{ to:viewerId, from:'host', candidate:candidate.toJSON() } })
    }
    ...
    // Create offer immediately after adding tracks — never use onnegotiationneeded
    pc.createOffer({ offerToReceiveAudio: false, offerToReceiveVideo: false })
      ...
      .then(() => {
        sigCh.current?.send({ type:'broadcast', event:'offer', payload:{
          to: viewerId, from: uRef.current?.id, sdp: pc.localDescription
        }})
      })
  ...
  // ── VIEWER: single RTCPeerConnection to host ────────────────────────
  const makeViewerPeer = (myId: string) => {
    ...
    pc.ontrack = e => {
      // Each track (video + audio) fires separately
      e.streams[0]?.getTracks().forEach(t => { if (!ms.getTrackById(t.id)) ms.addTrack(t) })
      if (remoteVid.current) {
        remoteVid.current.srcObject = ms
        // Try unmuted first; if browser blocks it, start muted and show prompt
        remoteVid.current.muted = false
        remoteVid.current.play().catch(() => {
          // Autoplay blocked — play muted then ask user to unmute
          if (remoteVid.current) {
            remoteVid.current.muted = true
            remoteVid.current.play().catch(() => {})
            setNeedsUnmute(true)
          }
        })
    ...
    pc.onicecandidate = ({ candidate }) => {
      if (!candidate || !sigCh.current) return
      sigCh.current.send({ type:'broadcast', event:'ice', payload:{ to:'host', from:myId, candidate:candidate.toJSON() } })
    }
    pc.onconnectionstatechange = () => {
    ...
    if (ctrl) {
      sig.on('broadcast', { event:'join' }, ({ payload }) => {
        ...
        if (!isLive.current || !camStream.current) {
          sig.send({ type:'broadcast', event:'not-live', payload:{ to:vid } }); return
        }
  ...
  const goLive = async () => {
    ...
    try {
      ...
      if (localVid.current) {
        ...
        localVid.current.muted     = true  // host preview always muted (no echo)
      ...
      // Connect viewers already present in the room
      const ps = chatCh.current?.presenceState() || {}
      Object.keys(ps).filter(v => v !== uRef.current?.id).forEach(vid => makeHostPeer(vid, stream))

      startRec(stream)
    } catch (err: any) {
      if      (err.name === 'NotAllowedError') setStreamErr('Camera/mic access denied. Please allow permissions.')
  ...
  const toCamera = () => {
    ...
    // Restore camera video track — audio was never replaced
    const vt = s.getVideoTracks()[0]; if (vt) replaceVid(vt)
    setMode('camera')
  }

  // Screen share — only replaces VIDEO track, audio track stays as cam mic
  const startScreen = async () => {
    try {
      ...
      // DO NOT replace audio — cam mic remains in all peer connection senders
    } catch {}
  }
  const stopScreen = () => { scrStream.current?.getTracks().forEach(t => t.stop()); scrStream.current = null; toCamera() }

  // Whiteboard — captures canvas as video stream, replaces only VIDEO track
  const startWhiteboard = (bg: string) => {
    ...
    setTimeout(() => {
      ...
      try {
        ...
        // Replace only VIDEO — audio (mic) stays untouched in all peer senders
        replaceVid(vt)
        // Host preview: show canvas video (muted since we don't want echo)
        if (localVid.current) { localVid.current.srcObject = ws; localVid.current.play().catch(() => {}) }
      } catch (e) {
        console.error('[WB] captureStream:', e)
        setStreamErr('Whiteboard requires Chrome 72+ or Edge.')
      }
    }, 200)
  }
  ...
  const sendMsg = async () => {
    ...
    if (hasBad(newMsg) && !(isHost || isCohost)) { alert('Message blocked by AutoMod.'); return }
  ...
  // ── Sub-render functions (NOT arrow components — avoids remount) ────────────
  ...
  const renderVideoArea = () => (
    <div ref={vidBox} style={{position:'relative',width:'100%',height:'100%',background:'#000',overflow:'hidden'}}>
      ...
      {canCtrl && (
        <video ref={localVid} autoPlay playsInline muted
          style={{width:'100%',height:'100%',objectFit:'cover',position:'absolute',inset:0,
            opacity: streaming && mode !== 'whiteboard' ? 1 : 0, transition:'opacity 0.2s'}}
        />
      )}
      {/* Viewer remote video */}
      {!canCtrl && (
      ...
      {/* Whiteboard canvas — fills video area when in whiteboard mode */}
      {mode === 'whiteboard' && canCtrl && (
        <div style={{position:'absolute',inset:0,zIndex:3,background:boardBg}}>
          <WhiteboardCanvas bg={boardBg} canvasRef={wbCanvas}/>
        </div>
      )}
        ...
        <div style={{position:'absolute',inset:0,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:16,background:'linear-gradient(180deg,#000 0%,#0A0F1E 100%)',zIndex:2}}>
          <div style={{width:80,height:80,borderRadius:'50%',overflow:'hidden',border:`3px solid ${uac(event?.users?.id||'')}44`,flexShrink:0}}>
            {event?.users?.photo_url
              ...
              : <div style={{width:'100%',height:'100%',background:uac(event?.users?.id||'')+'22',display:'flex',alignItems:'center',justifyContent:'center',fontSize:28,fontWeight:700,color:uac(event?.users?.id||''),fontFamily:'Syne,sans-serif'}}>{uinits(event?.users)}</div>
            }
          </div>
      ...
      {canCtrl && streaming && !camOn && mode==='camera' && (
        <div style={{position:'absolute',inset:0,display:'flex',alignItems:'center',justifyContent:'center',background:C.surface,zIndex:4}}>
          <div style={{textAlign:'center'}}>
            <div style={{width:56,height:56,borderRadius:'50%',background:uac(uRef.current?.id||'')+'22',display:'flex',alignItems:'center',justifyContent:'center',fontSize:20,fontWeight:700,color:uac(uRef.current?.id||''),margin:'0 auto 8px'}}>{uinits(pRef.current)}</div>
            <p style={{color:C.textMuted,fontSize:12,fontFamily:'DM Sans,sans-serif'}}>Camera off</p>
          </div>
        </div>
      )}

      {/* Tap to unmute overlay for viewer (browser autoplay policy) */}
  ...
  const renderHostBar = () => (
    <div style={{flexShrink:0,background:C.surface,borderTop:`1px solid ${C.border}`}}>
      {/* Whiteboard toolbar — shown inline below video when in whiteboard mode */}
      {streaming && mode === 'whiteboard' && (
        <WhiteboardToolbar
          bg={boardBg} canvasRef={wbCanvas} onClear={clearWhiteboard}
          tool={wbTool} setTool={setWbTool}
          color={wbColor} setColor={setWbColor}
          size={wbSize} setSize={setWbSize}
        />
      )}
      {/* Mode switcher row */}
      {streaming && (
        <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:6,padding:'8px 12px',borderBottom:`1px solid ${C.border}`,flexWrap:'wrap'}}>
          ...
          <div ref={wbDropRef} style={{position:'relative'}}>
            <button onClick={()=>setWbBgOpen(p=>!p)} style={{display:'flex',alignItems:'center',gap:5,padding:'6px 14px',borderRadius:8,border:`1px solid ${mode==='whiteboard'?'rgba(124,58,237,0.4)':'transparent'}`,background:mode==='whiteboard'?'rgba(124,58,237,0.15)':'rgba(51,65,85,0.4)',color:mode==='whiteboard'?'#A78BFA':C.textDim,fontSize:12,fontWeight:600,cursor:'pointer',fontFamily:'DM Sans,sans-serif'}}>
              <PenLine style={{width:13,height:13}}/>Whiteboard<ChevronDown style={{width:11,height:11,marginLeft:1}}/>
            </button>
            {wbBgOpen && (
              <div style={{position:'absolute',bottom:'calc(100% + 4px)',left:0,background:C.card,border:`1px solid ${C.border}`,borderRadius:10,overflow:'hidden',zIndex:50,minWidth:130,boxShadow:'0 8px 24px rgba(0,0,0,0.5)'}}>
                {[{n:'White',v:'#F8FAFC'},{n:'Dark',v:'#0A0A0F'},{n:'Green',v:'#064E3B'}].map(b => (
                  <button key={b.v} onClick={()=>{startWhiteboard(b.v);setWbBgOpen(false)}}
                    style={{display:'flex',alignItems:'center',gap:8,width:'100%',padding:'9px 14px',border:'none',background:boardBg===b.v&&mode==='whiteboard'?'rgba(124,58,237,0.15)':'transparent',color:boardBg===b.v&&mode==='whiteboard'?'#A78BFA':C.text,fontSize:12,fontWeight:600,cursor:'pointer',fontFamily:'DM Sans,sans-serif',textAlign:'left'}}>
