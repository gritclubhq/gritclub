...
const mod   = (t: string) => BAD.some(w => t.toLowerCase().includes(w))
const name  = (u: any) => u?.full_name || u?.email?.split('@')[0] || 'User'
const inits = (u: any) => name(u).slice(0, 2).toUpperCase()
const AC    = ['#2563EB','#7C3AED','#DB2777','#D97706','#059669','#0891B2']
const ac    = (id: string) => AC[(id?.charCodeAt(0) || 0) % AC.length]
const ago   = (ts: number) => { const m = Math.floor((Date.now()-ts)/60000); return m<1?'now':m<60?`${m}m`:`${Math.floor(m/60)}h` }
...
// ─── Whiteboard ───────────────────────────────────────────────────────────────
// canvasRef is passed from parent so captureStream() can send it via WebRTC
function Whiteboard({ bg, canvasRef }: { bg: string; canvasRef: React.RefObject<HTMLCanvasElement> }) {
  ...
  const up  = () => { drawing.current = false; origin.current = null }
  const clr = () => {
    const c = canvasRef.current; if (!c) return
    const ctx = c.getContext('2d')!; ctx.fillStyle = bg; ctx.fillRect(0, 0, c.width, c.height)
  }

  const TOOLS = [{id:'pen',I:PenLine},{id:'eraser',I:Eraser},{id:'line',I:Minus},{id:'rect',I:Square},{id:'circle',I:Circle}] as any[]
    ...
    <div style={{display:'flex',flexDirection:'column',width:'100%',height:'100%'}}>
      <div style={{display:'flex',alignItems:'center',gap:6,padding:'7px 12px',background:C.card,borderBottom:`1px solid ${C.border}`,flexWrap:'wrap',flexShrink:0}}>
        <div style={{display:'flex',gap:2,background:C.surface,padding:3,borderRadius:8}}>
          {TOOLS.map(({id,I}:any) => <button key={id} onClick={()=>setTool(id)} style={{width:28,height:28,borderRadius:6,border:'none',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',background:tool===id?C.blue:'transparent',color:tool===id?'#fff':C.textMuted}}><I style={{width:13,height:13}}/></button>)}
        </div>
        <div style={{display:'flex',gap:4}}>
          {COLS.map(col => <button key={col} onClick={()=>setColor(col)} style={{width:color===col?20:14,height:color===col?20:14,borderRadius:'50%',background:col,border:color===col?`2px solid ${C.blueL}`:'1px solid rgba(255,255,255,0.15)',cursor:'pointer',transition:'all .15s'}}/>)}
        </div>
        <div style={{display:'flex',gap:3,alignItems:'center'}}>
          {[2,4,8,14,22].map(s => <button key={s} onClick={()=>setSize(s)} style={{width:Math.max(s+4,10),height:Math.max(s+4,10),borderRadius:'50%',border:'none',cursor:'pointer',background:size===s?color:C.textDim}}/>)}
        </div>
        <button onClick={clr} style={{marginLeft:'auto',display:'flex',alignItems:'center',gap:4,padding:'4px 10px',borderRadius:6,border:'none',cursor:'pointer',background:C.redDim,color:C.red,fontSize:11,fontFamily:'DM Sans,sans-serif'}}>
...
export default function LiveRoomPage() {
  ...
  // ── Media refs ────────────────────────────────────────────────────
  const camStream = useRef<MediaStream|null>(null)   // camera + mic
  const scrStream = useRef<MediaStream|null>(null)   // screen share
  const wbStream  = useRef<MediaStream|null>(null)   // whiteboard canvas stream
  // txStream = what is currently being transmitted to all viewers
  const txStream  = useRef<MediaStream|null>(null)

  // ── WebRTC refs ───────────────────────────────────────────────────
  // host side: one RTCPeerConnection per viewer
  const hPeers = useRef<Map<string, {
    pc: RTCPeerConnection,
    iceQueue: RTCIceCandidateInit[]
  }>>(new Map())
  // viewer side: single connection to host
  const vPc    = useRef<RTCPeerConnection|null>(null)
  const vIce   = useRef<RTCIceCandidateInit[]>([])  // ICE queue before remote desc
  const isLive = useRef(false)   // true while host is broadcasting
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      // Muted users
      const { data: ml } = await supabase.from('live_muted_users')
        .select('user_id,muted_until').eq('event_id', eventId)
      if (ml) {
        const now = new Date()
        setMutedUsers(new Set(ml
          .filter(m => !m.muted_until || new Date(m.muted_until) > now)
          .map(m => m.user_id)) as Set<string>)
      }
      ...
      // Reactions
      const { count: rc } = await supabase.from('event_reactions')
        .select('*', { count:'exact', head:true }).eq('event_id', eventId)
      setReactions(rc || 0)
      const { data: myR } = await supabase.from('event_reactions')
        .select('id').eq('event_id', eventId).eq('user_id', u.id).maybeSingle()
      ...
      if (hc) {
        const { data: tix } = await supabase.from('tickets')
          .select('amount').eq('event_id', eventId).eq('status', 'paid')
      ...
      const ch = supabase.channel(`chat-${eventId}`, { config: { presence: { key: u.id } } })
        ...
        .on('broadcast', { event: 'hand' }, ({ payload }) => {
          setHands(prev => payload.raised
            ? [...prev.filter(h => h !== payload.uid), payload.uid]
            : prev.filter(h => h !== payload.uid))
        })
  ...
  const stopAllTracks = () => {
    ...
    camStream.current = null; scrStream.current = null
    wbStream.current = null;  txStream.current = null
  }

  // Replace video track on every viewer peer connection
  ...
  // Replace audio track on every viewer peer connection
  const replaceAud = (track: MediaStreamTrack) => {
    hPeers.current.forEach(({ pc }) => {
  ...
  // ── HOST: create one peer connection per viewer ───────────────────
  // Key fix: we manually create+send the offer instead of using
  // onnegotiationneeded which is unreliable and fires at wrong times.
  const makeHostPeer = (viewerId: string, stream: MediaStream) => {
    ...
    // Add all tracks from the active stream
    stream.getTracks().forEach(t => pc.addTrack(t, stream))

    pc.onicecandidate = ({ candidate }) => {
      if (!candidate || !sigCh.current) return
      sigCh.current.send({ type: 'broadcast', event: 'ice', payload: {
        to: viewerId, from: 'host', candidate: candidate.toJSON()
      }})
    }

    pc.onconnectionstatechange = () => {
      if (pc.connectionState === 'failed' || pc.connectionState === 'closed') {
        hPeers.current.delete(viewerId); pc.close()
      }
    }
    pc.oniceconnectionstatechange = () => { if (pc.iceConnectionState === 'failed') pc.restartIce() }

    // Manually create offer and send it — do NOT use onnegotiationneeded
    pc.createOffer({ offerToReceiveAudio: false, offerToReceiveVideo: false })
      .then(offer => pc.setLocalDescription(offer))
      .then(() => {
        sigCh.current?.send({ type: 'broadcast', event: 'offer', payload: {
          to:   viewerId,
          from: uRef.current?.id,
  ...
  // ── VIEWER: create peer connection to host ────────────────────────
  const makeViewerPeer = (myId: string) => {
    ...
    pc.ontrack = e => {
      // Add each incoming track — this fires once per track (video, audio separately)
      e.streams[0]?.getTracks().forEach(t => {
        if (!ms.getTrackById(t.id)) ms.addTrack(t)
      })
      // Always (re)assign srcObject so video element picks up new tracks
      if (remoteVid.current) {
        remoteVid.current.srcObject = ms
        remoteVid.current.play().catch(() => {})
      }
      setVStatus('connected')
    ...
    pc.onicecandidate = ({ candidate }) => {
      if (!candidate || !sigCh.current) return
      sigCh.current.send({ type: 'broadcast', event: 'ice', payload: {
        to: 'host', from: myId, candidate: candidate.toJSON()
      }})
    }

    pc.onconnectionstatechange = () => {
      if (pc.connectionState === 'connected') {
        setVStatus('connected'); setRetries(0); clearTimeout(retryT.current)
      }
  ...
  const setupSignaling = (uid: string, ctrl: boolean, alreadyLive: boolean) => {
    ...
    if (ctrl) {
      ...
      sig.on('broadcast', { event: 'join' }, ({ payload }) => {
        ...
        if (!vid || vid === uid) return
        if (!isLive.current || !txStream.current) {
          sig.send({ type: 'broadcast', event: 'not-live', payload: { to: vid } })
          return
        }
        makeHostPeer(vid, txStream.current)
      })

      // Viewer answered our offer
      sig.on('broadcast', { event: 'answer' }, async ({ payload }) => {
        if (payload.to !== uid && payload.to !== 'host') return
        const entry = hPeers.current.get(payload.from)
        if (!entry) return
      ...
      sig.on('broadcast', { event: 'ice' }, async ({ payload }) => {
        ...
        if (entry.pc.remoteDescription) {
          ...
          // Queue until answer is processed
      ...
      sig.on('broadcast', { event: 'offer' }, async ({ payload }) => {
        ...
        const pc = makeViewerPeer(uid)
        setVStatus('connecting')
        try {
          await pc.setRemoteDescription(new RTCSessionDescription(payload.sdp))
          // Flush ICE that arrived before the offer
          for (const c of vIce.current) { try { await pc.addIceCandidate(new RTCIceCandidate(c)) } catch {} }
          vIce.current = []
          const ans = await pc.createAnswer()
          await pc.setLocalDescription(ans)
          sig.send({ type: 'broadcast', event: 'answer', payload: {
            to:  payload.from,
            from: uid,
            sdp: pc.localDescription,
          }})
      ...
      sig.on('broadcast', { event: 'ice' }, async ({ payload }) => {
        if (payload.to !== uid) return
        if (vPc.current?.remoteDescription) {
          try { await vPc.current.addIceCandidate(new RTCIceCandidate(payload.candidate)) } catch {}
        } else {
    ...
    sig.subscribe(async s => {
      if (s === 'SUBSCRIBED' && !ctrl && alreadyLive) {
        ...
        // Retry once after 7 seconds if still not connected
        retryT.current = setTimeout(() => {
  ...
  const goLive = async () => {
    ...
    try {
      ...
      // Connect to any viewers already in the presence channel
      const ps = chatCh.current?.presenceState() || {}
      Object.keys(ps).filter(v => v !== uRef.current?.id).forEach(vid => {
        makeHostPeer(vid, stream)
      })

      startRec(stream)
    } catch (err: any) {
      if      (err.name === 'NotAllowedError') setStreamErr('Camera/mic access denied. Allow permissions in your browser.')
  ...
  // Step 1: setMode('whiteboard') → React renders the <Whiteboard> with wbCanvas ref
  // Step 2: after 200ms (canvas is painted), call canvas.captureStream(30)
  // Step 3: replaceVid() swaps the video track on every viewer peer — they now see the whiteboard live
  const startWhiteboard = (bg: string) => {
    scrStream.current?.getTracks().forEach(t => t.stop()); scrStream.current = null
    wbStream.current?.getTracks().forEach(t  => t.stop()); wbStream.current  = null
  ...
  const stopRec = async () => {
    ...
    const { error } = await supabase.storage.from('event-recordings')
    ...
    if (!error) {
      ...
      await supabase.from('event_recordings').insert({
        event_id: eventId, host_id: uRef.current?.id, storage_path: path,
        public_url: ud?.signedUrl || '', size_bytes: blob.size, premium_only: true,
      })
  ...
  const runMod = async (cmd: string) => {
    ...
    if (action === '/mute' && ct) {
      ...
      if (u) {
        ...
        setMutedUsers(p => new Set([...p, u.id]))
      ...
      if (u) {
        ...
        setBannedUsers(p => new Set([...p, u.id]))
        chatCh.current?.send({ type:'broadcast', event:'mod', payload:{ action:'ban', target:u.id } })
      }
    } else if (action === '/slow') {
      setSlowMode(true); chatCh.current?.send({ type:'broadcast', event:'mod', payload:{ action:'slow', enabled:true } })
    } else if (action === '/slowoff') {
      setSlowMode(false); chatCh.current?.send({ type:'broadcast', event:'mod', payload:{ action:'slow', enabled:false } })
    }
    setModCmd('')
  }
  ...
  const sendMsg = async () => {
    ...
    if (mod(newMsg) && !(isHost || isCohost)) { alert('Message blocked by AutoMod.'); return }
    if (mutedUsers.has(u?.id || '')) { alert('You are muted.'); return }
    setSending(true)
    const m = {
      id: crypto.randomUUID(), user_id: u?.id,
      name: name(p || u), avatar: p?.photo_url || '',
      text: newMsg.trim().slice(0, 500), ts: Date.now(),
  ...
  const toggleHand = () => {
    ...
    chatCh.current?.send({ type:'broadcast', event:'hand', payload:{ uid:uRef.current?.id, name:name(pRef.current||uRef.current), raised } })
  }
  ...
  // ── Render ────────────────────────────────────────────────────────
  // IMPORTANT: VideoArea, HostBar, ViewerBar, ChatPanel, TopBar are defined
  // outside JSX as regular functions so they don't re-mount on every render.
  // Defining them as arrow functions inside the return would destroy the
  // video element's srcObject every render.

  const renderVideoArea = () => (
    <div ref={vidBox} style={{position:'relative',width:'100%',height:'100%',background:'#000',overflow:'hidden'}}>

      {/* HOST preview — srcObject is always set to txStream (camera / screen / whiteboard canvas stream) */}
      {canCtrl && (
        <video ref={localVid} autoPlay playsInline muted
          style={{width:'100%',height:'100%',objectFit:'cover',position:'absolute',inset:0,
            opacity: (streaming && mode !== 'whiteboard') ? 1 : 0,
      ...
      {/* WHITEBOARD canvas — only rendered for host to draw on.
          captureStream(30) pipes drawing to all viewers via replaceVid() */}
      {mode === 'whiteboard' && canCtrl && (
        <div style={{position:'absolute',inset:0,zIndex:3}}>
          <Whiteboard bg={boardBg} canvasRef={wbCanvas}/>
        </div>
      )}
      ...
      {((canCtrl && !streaming) || (!canCtrl && vStatus !== 'connected')) && mode !== 'whiteboard' && (
        <div style={{position:'absolute',inset:0,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:16,background:'linear-gradient(180deg,#000 0%,#0A0F1E 100%)',zIndex:2}}>
          <div style={{width:80,height:80,borderRadius:'50%',overflow:'hidden',border:`3px solid ${ac(event?.users?.id||'')}44`,flexShrink:0}}>
            {event?.users?.photo_url
              ? <img src={event.users.photo_url} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/>
              : <div style={{width:'100%',height:'100%',background:ac(event?.users?.id||'')+'22',display:'flex',alignItems:'center',justifyContent:'center',fontSize:28,fontWeight:700,color:ac(event?.users?.id||''),fontFamily:'Syne,sans-serif'}}>{inits(event?.users)}</div>
            }
          </div>
          <div style={{textAlign:'center',maxWidth:300,padding:'0 20px'}}>
            {canCtrl ? (
              ...
              <>
                <p style={{color:'#fff',fontWeight:600,fontSize:15,fontFamily:'Syne,sans-serif',marginBottom:6}}>{name(event?.users)}</p>
                {vStatus==='connecting' && <>
                  ...
                  {retries>0 && <p style={{color:C.textDim,fontSize:11,marginTop:10,fontFamily:'DM Sans,sans-serif'}}>Retry {retries}...</p>}
                </>}
      ...
      {canCtrl && streaming && !camOn && mode==='camera' && (
        <div style={{position:'absolute',inset:0,display:'flex',alignItems:'center',justifyContent:'center',background:C.surface,zIndex:4}}>
          <div style={{textAlign:'center'}}>
            <div style={{width:56,height:56,borderRadius:'50%',background:ac(uRef.current?.id||'')+'22',display:'flex',alignItems:'center',justifyContent:'center',fontSize:20,fontWeight:700,color:ac(uRef.current?.id||''),margin:'0 auto 8px',fontFamily:'Syne,sans-serif'}}>{inits(pRef.current)}</div>
      ...
      {/* Raised hands */}
      {hands.length>0 && canCtrl && (
        <div style={{position:'absolute',bottom:12,left:12,zIndex:5,padding:'6px 12px',borderRadius:20,background:'rgba(245,158,11,0.2)',border:'1px solid rgba(245,158,11,0.4)',backdropFilter:'blur(8px)'}}>
          <span style={{fontSize:12,color:C.gold,fontFamily:'DM Sans,sans-serif'}}>✋ {hands.length} hand{hands.length>1?'s':''} raised</span>
        </div>
      )}
    </div>
  )
  ...
  const renderViewerBar = () => (
    <div style={{flexShrink:0,background:C.surface,borderTop:`1px solid ${C.border}`,padding:'10px 16px',display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}}>
      ...
      <div style={{flex:1}}/>
      {isMobile && (
        <button onClick={()=>{setShowChat(true);setUnread(0)}} style={{position:'relative',display:'flex',alignItems:'center',gap:5,padding:'8px 14px',borderRadius:20,border:`1px solid ${C.border}`,background:C.card,color:C.blueL,cursor:'pointer',fontSize:12,fontFamily:'DM Sans,sans-serif'}}>
          ...
          <div key={m.id} style={{display:'flex',gap:8}}>
            ...
            <div style={{flex:1,minWidth:0}}>
              <div style={{display:'flex',alignItems:'center',gap:5,marginBottom:2}}>
                ...
                <span style={{fontSize:10,color:C.textDim,fontFamily:'DM Sans,sans-serif'}}>{ago(m.ts)}</span>
      ...
      <div style={{padding:'10px 12px',borderTop:`1px solid ${C.border}`,flexShrink:0}}>
        ...
        <div style={{display:'flex',gap:8}}>
          <input type="text" value={newMsg} onChange={e=>setNewMsg(e.target.value)}
            ...
            placeholder={mutedUsers.has(uRef.current?.id||'')?'You are muted':'Say something...'}
            maxLength={500} disabled={mutedUsers.has(uRef.current?.id||'')||slowCD>0}
  ...
  const renderTopBar = () => (
    <div style={{padding:'10px 16px',background:C.card,borderBottom:`1px solid ${C.border}`,display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0}}>
      ...
      <div style={{display:'flex',alignItems:'center',gap:10,flexShrink:0}}>
        ...
        {!isMobile && !showChat && (
          <button onClick={()=>{setShowChat(true);setUnread(0)}} style={{display:'flex',alignItems:'center',gap:5,padding:'6px 12px',borderRadius:8,border:`1px solid ${C.border}`,background:'transparent',color:C.blueL,cursor:'pointer',fontSize:12}}>
    ...
    <div style={{height:'100dvh',display:'flex',flexDirection:'column',overflow:'hidden',background:C.bg}}>
      ...
      {isMobile && (
        <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden',position:'relative'}}>
          ...
          {!showChat && !canCtrl && (
            <button onClick={()=>{setShowChat(true);setUnread(0)}} style={{position:'absolute',bottom:72,right:16,zIndex:20,width:48,height:48,borderRadius:'50%',background:C.blue,border:'none',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',boxShadow:'0 4px 16px rgba(37,99,235,0.45)'}}>
      ...
      {!isMobile && !isTablet && (
        <div style={{flex:1,display:'flex',overflow:'hidden'}}>
          ...
          {showChat && (
            <div style={{flex:'0 0 30%',borderLeft:`1px solid ${C.border}`,display:'flex',flexDirection:'column',overflow:'hidden'}}>{renderChat()}</div>
          )}
        </div>
      )}
    </div>
