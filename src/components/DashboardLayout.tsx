...
function SignOutModal({ onConfirm, onCancel }: { onConfirm: () => void; onCancel: () => void }) {
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{ position: 'relative', width: '100%', maxWidth: 360, margin: '0 16px', borderRadius: 16, padding: 28, background: C.card, border: `1px solid ${C.border}`, boxShadow: '0 24px 64px rgba(0,0,0,0.8)' }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: 18 }}>
          <div style={{ width: 48, height: 48, borderRadius: '50%', background: C.redDim, border: `1px solid rgba(255,69,58,0.2)`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <LogOut style={{ width: 20, height: 20, color: C.red }} />
          </div>
          <div>
            <p style={{ fontFamily: C.fontSora, fontWeight: 600, color: C.text, marginBottom: 6, fontSize: 15 }}>Sign out of GritClub?</p>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '18px 14px 14px', borderBottom: `1px solid ${C.border}`, flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 28, height: 28, borderRadius: 7, flexShrink: 0, background: '#FFFFFF', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: C.fontSora, fontWeight: 800, fontSize: 12, color: '#000' }}>G</div>
          <div>
            <div style={{
              fontFamily: C.fontSora, fontWeight: 800, fontSize: 14, letterSpacing: '-0.02em',
              background: 'linear-gradient(135deg, #E8E8E8 0%, #CFCFCF 25%, #FFFFFF 50%, #B8B8B8 75%, #EDEDED 100%)',
              WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text',
            }}>GRITCLUB</div>
            {role && role !== 'audience' && (
              <span style={{ display: 'inline-block', marginTop: 2, fontSize: 9, fontFamily: C.fontInter, fontWeight: 600, padding: '1px 6px', borderRadius: 3, letterSpacing: '0.1em', textTransform: 'uppercase', background: role === 'admin' ? C.redDim : 'rgba(255,255,255,0.06)', color: role === 'admin' ? C.red : C.textMuted, border: `1px solid ${role === 'admin' ? 'rgba(255,69,58,0.2)' : C.border}` }}>{role}</span>
            )}
          </div>
        </div>
      </div>
      ...
      <nav style={{ flex: 1, overflowY: 'auto', padding: '8px 6px' }}>
        ...
        <div style={{ marginTop: 10, paddingTop: 10, borderTop: `1px solid ${C.border}` }}>
          <Link href="/pricing" onClick={() => setMobileOpen(false)} style={{ textDecoration: 'none', display: 'block' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '8px 10px', borderRadius: 8, color: C.textDim, fontFamily: C.fontInter, fontSize: 13, cursor: 'pointer', transition: 'background 0.15s' }}
              onMouseEnter={e => (e.currentTarget as HTMLElement).style.background = 'rgba(255,255,255,0.03)'}
              onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = 'transparent'}>
              <Crown style={{ width: 14, height: 14, flexShrink: 0, opacity: 0.5 }} />
      ...
      <div style={{ flexShrink: 0, padding: '6px 6px 8px', borderTop: `1px solid ${C.border}` }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '9px 10px', borderRadius: 8, background: 'rgba(255,255,255,0.025)', marginBottom: 2 }}>
          <div style={{ width: 30, height: 30, borderRadius: '50%', flexShrink: 0, overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#FFFFFF', fontFamily: C.fontSora, fontWeight: 700, fontSize: 11, color: '#000' }}>
            {photoUrl ? <img src={photoUrl} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : initials}
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <p style={{ fontSize: 12, fontWeight: 500, color: C.text, fontFamily: C.fontSora, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', margin: 0 }}>{displayName}</p>
        ...
        <button onClick={() => setShowSignOut(true)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 9, padding: '7px 10px', borderRadius: 8, border: 'none', cursor: 'pointer', background: 'transparent', color: C.textDim, fontFamily: C.fontInter, fontSize: 12, fontWeight: 400, transition: 'all 0.15s' }}
    ...
    <>
      ...
      {mobileOpen && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 50 }}>
          <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(4px)' }} onClick={() => setMobileOpen(false)} />
      ...
      <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: C.bg }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
          ...
          <div className="gc-mobile-top" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 14px', height: 52, background: C.sidebar, borderBottom: `1px solid ${C.border}`, flexShrink: 0 }}>
            ...
            <span style={{
              fontFamily: C.fontSora, fontWeight: 800, fontSize: 16,
              background: 'linear-gradient(135deg, #E8E8E8 0%, #CFCFCF 25%, #FFFFFF 50%, #B8B8B8 75%, #EDEDED 100%)',
              WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text',
            }}>GRITCLUB</span>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              {user && <NotificationBell userId={user.id} />}
              <div style={{ width: 28, height: 28, borderRadius: '50%', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#FFFFFF', fontSize: 10, fontWeight: 700, color: '#000', fontFamily: C.fontSora }}>
                {photoUrl ? <img src={photoUrl} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : initials}
              </div>
            </div>
          </div>
